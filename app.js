...
let allTodos = []; // all todos in an array

todoForm.addEventListener('submit', function(e){  //taking input
  e.preventDefault(); // will not load autimatically
  addTodo(); 
})
function addTodo(){
    const todoText = todoInput.value.trim(); //variable = assign a value
    if(todoText.length > 0){

         allTodos.push(todoText); //adding todos to array
         updateTodoList();
         todoInput.value =""; //setting toso to an empty string
    }
